...
Using an anova test between models that either had or didn't have Site as a categorical predictor variable. The test resulted in a p-value well below the significance level of `0.05` which meant they were statistically significantly different. Comparing the model summaries, we saw that when Site was included we had an $R^2 =.80$ compared to the model with no Site, which returned $R^2 = .64$ . Including the categorical variable of site allowed for us to explain roughly 16% more of the data. Additionally, we compared models where the categorical predictor variable were Site and Population. By performing an anova test which resulted in a statistically significant difference. The Population model had an $R^2 =.74$ which was less than the aforementioned value of the Site model. We also compared the Site model's $Adjusted\ R^2 =.79$ with the Population's $Adjusted\ R^2 =.73$ . Using both those metrics the Site model was able to explain more of the data while still being better suited to generalize potential new data.
...
One of the most important metrics used in comparing linear models is the $R^2$. Previously, this metric was found and compared between the Site and Population models, they returned values of 0.80 and 0.74 respectively. Our findings might suggest, If a site had more food or a harsher environment possums potenital could be larger there. Due to abundance of nutrients or the need to defend from predators. However, since we have no additional data we are merely speculating on the potential reason why Site as a predictor variable allowed models to perform better.
