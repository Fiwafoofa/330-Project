...
Our data set involves the measurements of a species of possum over seven sites in Australia, ranging from Southern Victoria to Queensland, which encompasses almost all of Australia's eastern coast.
...
Next is the assumption that the residuals of the model are normally distributed. Looking at the histogram of residuals and the Shapiro Test, the histogram is normally distributed and the null hypothesis that the residuals are normally distributed is not rejected. This assumption is easily met.
...
The first assumption checked is no multicollinearity among the predictor variables. The correlation matrix and VIFs were computed. In the correlation matrix, none of the variables have a higher correlation than `0.8`, which is ideal, and the mean and max of the VIFs was `1.90` and `2.08` respectively, which is less ideal but likely still good enough. The assumption is likely met and will not interfere much with the predictions of the model.

Finally is the check for influential points. Looking at both residuals vs. leverage plots, all data points were within bounds (less than `0.5`), meaning that none of them could be considered influential.
...
To answer the initial questions about our data set, an analysis and other tests on the models were performed using 2 different models with variations on those models. These two models are the Site model and Population model. The Site model was a linear model that used *site*, *chest*, *head length* and *tail length* as predictor variables with the response variable being Total Length (See Figure 4). The Population model was a similar linear model that only used *population*, *tail length*, and *head length* as predictors and Total Length as the response (See Figure 6).

First, an anova test was conducted to see if the inclusion of the *population* and *site* variables were producing statically significant different models. In other words, the anova test checked to see how the addition of a categorical variable, *population* or *site*, affected the model. We tested models that included either Site or Population as their categorical variable. For the model that contained site, our anova test produced a p-value of $7.604e^{-11}$ suggesting that the Site variable (along with *chest*, *head* *length*, and *tail* *length*) is a statistically significant predictor variable in this model (See Figure 5). Next we conducted an anova test including the Population in the model, resulting in a p-value of $6.395^{-10}$. This indicates that for a model with Tail Length and Head Length, Population is a statically significant predictor variable (See Figure 7). The results of the anova tests are shown below.

To further understand the relationship that Site and Population have as predictor variables in a linear model, we conducted an anova test comparing models that contained the respective variables as predictors. The test resulted in a p-value of $2.747e^{-5}$ . Which indicates that the difference between the models is statistically significant at an alpha level of 0.05. The test is shown below.

Now that we have compared the models, we can look at how they actually are structured. For this section, we will compare the predictor variables that both models share (Tail Length and Head Length), what effect they have, and compare the difference categorical variables make in the models.
